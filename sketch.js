			...
			if(connectRouters[0] !== connectRouters[1])
			{
				...
				{ 
					...
					{	
						...
						connectors.push(
							new Line(v2, v1, connectRouters[1], connectRouters[0])
							);
...
function doubleClick() 
{
	...
	{ 
		if(isOnEdge(new Vector(mouseX, mouseY), connectors[i])) 
		{
			...
			var cost = prompt("Please enter path cost from "+ connectors[i].startRouter.name +" to "+ connectors[i].endRouter.name + ":", connectors[i].weight); 

			if(isNaN(cost) || cost === "" || !cost) 
			{  
				alert("You have to put numberical value in this field." 
					 + "\nCost will be set to " + connectors[i].weight + " by default"); 
			}
			else 
			{ 
...
{ 
	...
	if(!inArray(routers, sourceName)) 
	{ 
		alert("Please set a source node!");
		return;
	}
	if(!inArray(routers, destName)) { 
		fullRouteFromSrc = true; 
	}
	else { 
...
function createGUI() 
{
	var gui = new dat.GUI({ autoPlace: true, name: "Dijkstra", width: 250, closeOnTop:true });
	
	// console.log(gui.domElement.style); 
	gui.domElement.style.float = "left";

	gui.add(window, 'showHelp').name("Help"); 
	gui.add(Modes, 'graph', ['Undirected Graph', 'Directed Graph']).name("Graph").onChange(()=>{directed = !directed}); 
...
{ 
	...
	{
		for(var i=0; i<connectors.length; i=i+2) 
		{ 
			var start = getIndex(routers, connectors[i].startRouter); 
			var end = getIndex(routers, connectors[i].endRouter); 
			
			routersCostMap[start][end] = connectors[i].weight; 
			routersCostMap[end][start] = connectors[i+1].weight; 
