...
{ 
	...
	{ 
		...
		{
			x = connectors[i].begin.x - (connectors[i].begin.x - connectors[i].end.x)/4; 
			y = connectors[i].begin.y - (connectors[i].begin.y - connectors[i].end.y)/4; 	
			ellipse(x, y, edgeEllipseRadius, edgeEllipseRadius);
		}
...
function drawPermanent() 
{
	...
	for(var item of permanent) 
	{
		...
		ellipse(x+20, y+20, edgeEllipseRadius, edgeEllipseRadius);
		...
		if(directed) {
			textSize(10); 
			fontWidth = textWidth(String(edge.endRouter.distance));
			text(edge.endRouter.distance, x - fontWidth/2 + 20, y + 4 + 20);
		} 
		else {
			textSize(10); 
			fontWidth = textWidth(String(edge.endRouter.distance));
			if(edge.startRouter.distance > edge.endRouter.distance) { 
				text(edge.startRouter.distance, x - fontWidth/2 + 20, y + 4 + 20);
			}
			else {
				text(edge.endRouter.distance, x - fontWidth/2 + 20, y + 4 + 20); 
			}
	...
	/* for(var i=0; i<guiRoutesUpdate.length; i++) { 
		...
		table += "\tcost: "+ getEdge(guiRoutesUpdate[i]) + "\n"; 
	} */
	for(var item of permanent) 
	{
		if(item === undefined) break; 
		if(item.prev === -1) 
			continue;
		table += item.prev.name + " - " + item.name + " Cost: " + getEdge(item.prev, item).weight + "\n"; 
	}
...
{ 
	...
	while(routers[temp].prev !== -1) 
	{
		...
		edge = getEdge(routers[temp].prev, routers[temp]);  
		x = edge.end.x + (edge.begin.x - edge.end.x)/2; 
		y = edge.end.y + (edge.begin.y - edge.end.y)/2; 
	...
	for(var i= destSrcTable.length-1; i >= 0; i--) { 
		...
		name = 	((routers[destSrcTable[i]].prev.name !== undefined) ? 
				routers[destSrcTable[i]].prev.name : "S") + " - " +  routers[destSrcTable[i]].name;  
